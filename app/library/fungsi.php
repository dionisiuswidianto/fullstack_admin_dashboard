<?php  
	...
	class oop{
		...
		function ubah($table, array $field,$where,$alamat){
			...
			$jalan = mysql_query($sql);
			if($jalan){
				echo "<script>alert('Data Berhasil Diubah');document.location.href='$alamat'</script>";
			}else{
				echo mysql_error();
			}
		}
		...
		function ceklogin($home,$tipe){
			if (empty($_SESSION['user']) || empty($_SESSION['type'])) {
				echo "<script>alert('Harap Melakukan Login Terlebih dahulu');document.location.href='$home';</script>";
				// header("location:$home");
			}else{
				if ($_SESSION['type']!==$tipe) {
					echo "<script>alert('Anda tidak dapat akses ke halaman ini');document.location.href='$home';</script>";
				}
			}
		}
